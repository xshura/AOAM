...
class DDPG:
    ...
    def update_models(self, states, actions, critic_target, sess):
        ...
        # Train actor
        self.actor.train(states, actions, np.array(grads).reshape((-1, self.act_dim)), sess)
        # Transfer weights to target networks at rate Tau
        self.actor.transfer_weights()
        self.critic.transfer_weights()

    def train(self, env, args, summary_writer, graph, sess):
        results = []
        ...
        for e in tqdm_e:
            ...
            while not done:
                ...
                a = a * self.act_range
                a = np.clip(a+noise.generate(time), 0, self.act_range)
                # Retrieve new state, reward, and whether the state is terminal
                # print('action:', a)
                new_state, r, done, _ = env.step(a)
                # Add outputs to memory buffer
                f = open('{}_action.txt'.format(args.dataset), 'a')
                f.write('score:{}'.format(r) + "  " + str(env.actions))
                f.close()
                ...
                with graph.as_default():
                    ...
                    self.update_models(states, actions, critic_target, sess)
