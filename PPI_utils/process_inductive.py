...
def load_ppi(dataset):
    print('Loading G...')
    with open('data/{}/{}-G.json'.format(dataset, dataset)) as jsonfile:
    ...
    with open('data/{}/{}-id_map.json'.format(dataset, dataset)) as jsonfile:
    ...
    features_ = np.load('data/{}/{}-feats.npy'.format(dataset, dataset))
    ...
    class_map = {}
    with open('data_{}/{}-class_map.json'.format(dataset, dataset)) as jsonfile:
    ...
    print("split graphs in reddit", len(nodes_per_graph), subgraph_nodes, subgraph_nodes)
    ...
    for i in range(len(test_nodes)):
        for j in range(test_nodes[i]):
            ts_msk[i][j] = 1
    A, features, y_train, y_val, y_test, idx_train, idx_val, idx_test, train_mask = adj_sub, feat_sub, train_labels, val_labels, test_labels, train_nodes, val_nodes, test_nodes, tr_msk
    ...
    return A, features, y_train, y_val, y_test, idx_train, idx_val, idx_test, train_mask
